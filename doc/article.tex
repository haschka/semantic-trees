...
to decide from the previous output on a single research field.
...
overarching research field encompassing all $s$ subfields. 
...
\begin{figure*}
  ...
  \caption{A comparison of trees built from the 20-newsgroup dataset
...
Depending on the dataset we used different evaluation methods, in
order to validate if, and to what kind of extend our in here proposed
method works, under the computational constraints that are given to
us. For a quick evaluation that topic attibution in the tree built
from the emebddings works we outline our trees built from the
20-newsgroups dataset in Figure \ref{fig-20news-colored}.
The inferred tree for the 20 Newsgroups dataset uses
color to indicate how strongly each cluster corresponds to a
particular newsgroup. Posts belonging to the target newsgroup
are shown in blue, while posts from all other newsgroups are
shown in orange. Both colors vary on a logarithmic scale based
on how many posts of each type appear in the cluster: white
indicates zero posts, and deeper blue or orange indicates a
higher concentration, reaching full blue when all posts in the
cluster belong to the target newsgroup, and full orange when all
posts come from other newsgroups.

We see that different newsgroups with supposely similar content enter the same
tree branches. 
\begin{figure*}
  \begin{center}
